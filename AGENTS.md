...
These standards apply to every change, in addition to the project-specific conventions below.

- **Clean, elegant code that does not sacrifice performance.** Prefer small, well-named functions and minimal public APIs (see the opening paragraph). The crate is an inference runtime with a vectorised hot path, so performance is a feature: keep allocation out of `activate`, hoist invariant work out of inner loops, and prefer `Simd` gathers/scatters over scalar loops on the connection sweep. When clarity and speed conflict, *first* reconsider the algorithm — the existing design shows that readable safe code can be fast (no `unsafe`, `portable_simd`, monomorphised traits). Only after ruling out a better algorithm may you locally trade a little clarity for speed, and the resulting code must carry a comment explaining why. Never add `unsafe` (see project facts).
- **Extensive tests for every functionality implemented.** New public API, new behaviour, or a bug fix comes with tests that pin down *behaviour*, not implementation. Cover the happy path, edge cases (empty inputs, zero, extremes, boundary IDs), and error paths. Float comparisons use `<=` against a tolerance (see conventions). Integration tests that exercise the full parse → build → activate pipeline go in `tests/integration.rs`; unit tests live next to the code under `#[cfg(test)]`. A regression test must accompany every bug fix, demonstrating the failure before the fix and passing after. Run `cargo test` (plus the lint/verify suite above) before considering work done.
- **Benchmarks for perf-sensitive code.** Anything on the activation or network-activation hot path, or any change that could move throughput, gets a benchmark in `benches/` following the existing zero-dependency harness pattern (`std::time::Instant` + `std::hint::black_box`, warm-up then fixed measurement window, ns/call output). Add a `[[bench]]` entry to `Cargo.toml` with `harness = false` if you create a new bench file. Compare the enum-dispatch and concrete-monomorphised forms when relevant (the activation benches do this). Benchmarks are not run by `cargo test`; mention them in the PR/commit message so a reviewer knows to run them.
